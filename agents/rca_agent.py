...
- Good root_cause examples: "Network Packet Loss", "Memory Leak", "Database Pool Exhaustion", "Disk Full", "CrashLoopBackOff", "Certificate Expired", "CBS Manifest Invalid", "Invalid Package".
...
- If logs mention CBS_E_MANIFEST_INVALID_ITEM, 0x800f080d, or "Unrecognized packageExtended attribute", prefer root_cause "CBS Manifest Invalid".
- If logs mention CBS_E_INVALID_PACKAGE, 0x800f0805, or "Failed to internally open package", prefer root_cause "Invalid Package".
- If logs mention SQM upload failure along with CBS errors, treat SQM as a secondary symptom, not the primary root cause.
- For Windows CBS logs, blast_radius should include only Windows components directly present in logs, such as "CBS", "WindowsUpdateAgent", "SQM".
